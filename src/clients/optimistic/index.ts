...
export class OptimisticLightClient extends BaseClient {
  ...
  */
  async fight(prover1: ProverInfoL, prover2: ProverInfoL, period: number, prevCommitteeHash: Uint8Array): Promise<[boolean, boolean]> {
    let prevCommittee: Uint8Array[] | undefined;
    try {
    prevCommittee = period === this.genesisPeriod ? this.genesisCommittee : await this.getCommittee(period - 1, prover1.index, prevCommitteeHash)
    } catch (e) { prevCommittee = await this.getCommittee(period - 1, prover2.index, prevCommitteeHash); }
 ...
 */
  async syncFromGenesis(): Promise<ProverInfo[]> {
    return new Promise(async (resolve, reject) => {
      ...
      let lastCommitteeHash: Uint8Array | undefined = this.getCommitteeHash(this.genesisCommittee);
      ...
      if (!foundConflict) {
        ...
        resolve([{index: index, syncCommittee: committee}]);
      }
      proverInfos = proverInfos.filter((pi: ProverInfoL, i: number) => (committeeHashes[i] !== null));
      if (lastCommitteeHash !== undefined) {
        proverInfos = await this.tournament(proverInfos, period, lastCommitteeHash);
      }        
      if (proverInfos.length === 0) reject(new Error('none of the provers responded honestly :('));
      ...
      reject(new Error(`none of the provers responded honestly :( : ${e.message}`));
      }
      ...
      for (const p of proverInfos) {
        try {
          ...
          resolve([{index: p.index, syncCommittee: committee}]);
        } catch (e) {}
      }
      reject(new Error('none of the provers responded honestly :('));
    });
  }
}
